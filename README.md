...
Apply clones or fast-forwards the OpenCode-only learning system from `https://github.com/guisaliba/learn.git` into `~/.local/share/opencode/learn`. It runs `PUPPETEER_SKIP_DOWNLOAD=true bun install --frozen-lockfile` in that checkout, then merges the absolute local directory as the server entry in `~/.config/opencode/opencode.json` and the TUI entry in `~/.config/opencode/tui.json`. Both entries are required for graded quiz dialogs and immediate feedback. Each merge removes known stale GitHub entries and the managed local entry, keeps one managed entry, preserves unrelated valid settings, and is idempotent.

The managed checkout is separate from the development checkout at `~/projects/active/self/learn`. Apply does not modify the development checkout. An existing managed checkout must have the expected remote, the `main` branch, and a clean worktree. Apply refuses to update a dirty or unrelated checkout. Set `LEARN_REPOSITORY_URL` when the repository uses another authenticated Git URL, such as an SSH URL for a private GitHub repository. The user Git credentials must already allow `git clone` and `git fetch`.

The `learn` primary agent inherits the current session model. Its research and visual-maker subagents use the upstream model defaults shipped by the learn repository itself; apply configures the plugin path without text or visual model overrides. This is separate from the managed ai-memory service profile below.

Use `/learn <goal>` for frontier probing, a reviewed dependency plan, one-node teaching loops, graded quizzes, an optional live Markdown log, and inspected SVG or Mermaid teaching visuals. Mermaid rendering needs Chrome or Chromium. Apply does not download Puppeteer's managed browser because this setup uses an existing system browser through `CHROME_PATH` or a standard system path; this avoids a large duplicate browser download on every setup. Set `CHROME_PATH` when the browser is not in a standard location.
...
ai-jail is optional and is not a dependency of ai-memory or the normal managed OpenCode runtime. Apply does not install or configure it. Install it separately only when this sandboxed dangerous-mode path is required.
...
Do not vendor upstream skill, generated ai-memory instruction, or plugin payloads in this repo. The Learn payload is fetched into its machine-local managed checkout by `apply.sh`.
